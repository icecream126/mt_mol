...
import utils.auc
from utils.task_dicts import (
    get_task_to_condition_dict,
    get_task_to_dataset_path_dict,
    get_task_to_score_dict,
    get_task_to_scientist_prompt_dict,
    get_task_to_scientist_prompt_with_review_dict,
    get_task_to_reviewer_prompt_dict,
    get_task_to_scientist_prompt_with_double_checker_dict,
    get_task_to_double_checker_prompt_dict,
    get_task_to_functional_group_dict,
)
...
def run_pubchem_functions(
    function_names, smiles_list, utils_path="/home/anonymous/chemiloop/utils/tools.py"
):
    """
...
def update_best_score(
    current_score: float, current_smiles: str, best_score: float, best_smiles: str
) -> Tuple[float, str]:
...
def calculate_metrics(
    smiles_scores: List[Tuple[str, float]], iteration: int, max_iterations: int
) -> Dict[str, float]:
    """Calculate various metrics for the current state"""
    finish = iteration + 1 >= max_iterations
    auc_top10_all, _ = utils.auc.compute_topk_auc(
        smiles_scores,
        top_k=10,
    ...
    auc_top1_all, _ = utils.auc.compute_topk_auc(
        smiles_scores,
        top_k=1,
        max_oracle_calls=1000,
        freq_log=1,
...
def tool_electronic_topological_descriptors(state: GraphState) -> GraphState:
    ...
    tool_path = (
        "/home/anonymous/chemiloop/tool_jsons/Electronic_Topological_Descriptors.json"
    )
...
```"""
    ...
    with open(
        f"./logs/{config.current_time}-{config.run_id}/electronic_tools.txt", "a"
    ) as f:
...
def tool_fragment_based_functional_groups(state: GraphState) -> GraphState:
    """Node for selecting tools to use in the workflow"""
    tool_path = (
        "/home/anonymous/chemiloop/tool_jsons/Fragment_Based_Functional_Groups.json"
    )
...
```"""
    ...
    with open(
        f"./logs/{config.current_time}-{config.run_id}/fragment_based_tools.txt", "a"
    ) as f:
...
```"""
    ...
    with open(
        f"./logs/{config.current_time}-{config.run_id}/other_tools.txt", "a"
    ) as f:
...
```"""
    ...
    with open(
        f"./logs/{config.current_time}-{config.run_id}/structural_tools.txt", "a"
    ) as f:
...
def tool_identifiers_and_representations(state: GraphState) -> GraphState:
    """Node for selecting tools to use in the workflow"""
    tool_path = (
        "/home/anonymous/chemiloop/tool_jsons/Identifiers_and_Representations.json"
    )
...
```"""
    ...
    with open(
        f"./logs/{config.current_time}-{config.run_id}/identifier_tools.txt", "a"
    ) as f:
    ...
    state["tools_to_use"] = (
        state["electronic_tools"]
        + state["fragment_based_tools"]
        + state["other_tools"]
        + state["structural_tools"]
    )
    state["target_functional_groups"] = run_pubchem_functions(
        state["tools_to_use"] + state["identifier_tools"], state["parsed_smiles"]
    )
...
def scientist_node(state: GraphState) -> GraphState:
    ...
    try:
        ...
        state["scientist_think_dict"] = {
            key: parsed.get(key, "") for key in default_keys
        }
...
def double_checker_node(state: GraphState) -> GraphState:
    ...
    if not validate_smiles(state["generated_smiles"]):
        state["functional_groups"] = (
            "No functional groups because the SMILES is invalid."
        )
    else:
        state["functional_groups"] = run_pubchem_functions(
            state["tools_to_use"], state["generated_smiles"]
        )
...
def reviewer_node(state: GraphState) -> GraphState:
    ...
    state["best_score"], state["best_smiles"] = update_best_score(
        state["score"],
        state["generated_smiles"],
        state["best_score"],
        state["best_smiles"],
    ...
    metrics = calculate_metrics(
        state["smiles_scores"], state["iteration"], state["max_iterations"]
    )
    wandb.log(metrics)
...
def route_after_double_checker(state: GraphState) -> str:
    ...
    if (
        state["double_checker_think_dict"]["consistency"].strip().lower()
        == "consistent"
    ):
        logger.log(
            "Double checker consistency is consistent, returning to reviewer node"
        )
        DOUBLE_CHECKER_COUNT = 0
        IN_DOUBLE_CHECKING_PROCESS = False
        ...
        logger.log(
            f"Double checker consistency is inconsistent, returning to scientist node.\nDouble checker count: {DOUBLE_CHECKER_COUNT}"
        )
...
def create_graph() -> StateGraph:
    ...
    builder.add_conditional_edges(
        "reviewer_node", should_continue, {True: "scientist_node", False: END}
    )
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="test",
        name=args.task[0] if isinstance(args.task, list) else args.task,
        config=vars(args),
    )  #  , mode="disabled")
